...
- The resource must have at least a POST and a GET operations defined as shown in the example below. Update (PATCH) and 
...
- The resource's POST, GET and PATCH (if exposed) operations must have the same response schema configuration. This is required to ensure
the resource state is consistent.

- The resource's PATCH operation (if exposed) request payload must be the same as the resources POST's request schema. The OpenAPI provider expects this
 to ensure the update operation enables replacement of the representation of the target resource.

- The resource's PATCH operation may return one of the following successful responses:
...
object re-usability across the resource CRUD operations. The resource POST/GET/PATCH operations are expected to have a 'schema' property
  ...
  LBV1:
    ...
    properties:
      ...
      timeToProcess: # time that the resource will take to be processed in seconds
        type: integer
        default: 60 # it will take two minute to process the resource operation (POST/PATCH/READ/DELETE)
      simulate_failure: # allows user to set it to true and force an error on the API when the given operation (POST/PATCH/READ/DELETE) is being performed
        type: boolean
  Status:
    type: object
    properties:
...
The above configuration will make the OpenAPI Terraform provider client make API CRUD requests (POST/GET/PATCH/DELETE) to
...
resource such as GET/PATCH/DELETE will used the overridden host value too.*
...
CRUD operations. This means that, it is expected that the rest of the operations Read (GET), Update (PATCH) and Delete (DELETE)
...
x-terraform-field-status | boolean | If this meta attribute is present in a definition property, the value will be used as the status identifier when executing the polling mechanism on eligible async operations such as POST/PATCH/DELETE.
